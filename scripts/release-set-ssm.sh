...
s3_metadata="$( aws s3api head-object --bucket "${RELEASE_BUCKET}" --key "${RELEASE_PREFIX}/sha384/${RELEASE_ID}.tar.gz" )"
s3_size="$( jq -r '.ContentLength' <<< "${s3_metadata}" )"
[[ -z "${s3_size}" || "${s3_size}" == "null" ]] && { echo "error: ${RELEASE_PREFIX}/sha384/${RELEASE_ID}.tar.gz size: not in S3 bucket ${RELEASE_BUCKET}"; exit 1; }
[[ "${s3_size}" -le 1024 ]] && { echo "error: ${RELEASE_PREFIX}/sha384/${RELEASE_ID}.tar.gz size is <1kb in S3 bucket ${RELEASE_BUCKET}"; exit 1; }
s3_contenttype="$( jq -r '.ContentType' <<< "${s3_metadata}" )"
[[ "${s3_contenttype}" != "application/x-tar" ]] && { echo "error: ${RELEASE_PREFIX}/sha384/${RELEASE_ID}.tar.gz in S3 bucket ${RELEASE_BUCKET} has invalid content-type ${s3_contenttype} expected application/x-tar"; exit 1; }
